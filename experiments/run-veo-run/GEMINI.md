...
-   **Architecture:** See `docs/frontend_architecture.md` for component hierarchy and state management details.
...
2.  **Tool Usage:**
    ...
    *   **File Editing:** Use `write_file` for large files or files with complex template literals (like `lit-html` components) to avoid context matching errors with `replace`.

    *   **Task Tracking:** Use `bd` for all task tracking.
...
3.  **Refactoring:**
    ...
    *   Update `docs/frontend_architecture.md` if significant structural changes are made.
...
4.  **Security & Validation:**
    ...
    *   **Double Validation:** Enforce constraints (MIME type, duration, dimensions) on BOTH the Frontend (for UX) and Backend (for security).
...
## 7. Known Limitations & Workarounds (Veo 3.1)

*   **Input Limits:**

    *   **Max Duration:** 30 seconds (Hard limit for extension input).

    *   **MIME Type:** `video/mp4` only.

    *   **Resolution:** Must be 1280x720 (Landscape/Portrait) or 1920x1080 (Landscape/Portrait).
