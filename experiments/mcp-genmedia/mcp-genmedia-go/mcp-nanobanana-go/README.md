...
- `model` (string, optional): The specific NanoBanana (Gemini Image) model to use. Defaults to `gemini-1.5-pro-latest`.
...
Synthesizes speech from text using NanoBanana (Gemini Image) models, allowing for granular control over style, pace, tone, and emotional expression through natural-language prompts.
...
Lists the available single-speaker voices for use with the NanoBanana (Gemini Image)-TTS models.
...
mcptools call gemini_audio_tts \
  --params '{"text": "Hello, this is a test of the NanoBanana (Gemini Image) Text-to-Speech API.", "output_directory": "./tts_output"}' \
  mcp-nanobanana-go
```
