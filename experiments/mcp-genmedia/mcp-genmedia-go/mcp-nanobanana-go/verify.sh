...
response=$(mcptools tools ./mcp-nanobanana-go)

# Check if the response contains the expected tool name
if [[ "$response" == *"nanobanana_image_generation"* ]]; then
  echo "Verification successful: 'nanobanana_image_generation' tool found."
else
